...
<?php defined('KODE_KOTA_DEFAULT') or define('KODE_KOTA_DEFAULT', '1301') ?>

<?php $prayer_time = ['imsak', 'subuh', 'dzuhur', 'ashar', 'maghrib', 'isya'] ?>
<?php $is_shown = config_item('tampilkan_jadwal_shalat') ?? true ?>

<script>
  const KODE_KOTA = "<?= config_item('kode_kota') ?: KODE_KOTA_DEFAULT ?>";
...
<?php if($is_shown) : ?>
  <section class="flex flex-col lg:flex-row lg:space-x-5 lg:space-y-0 space-y-3 shalat">
    ...
    <div class="lg:w-2/3 grid grid-cols-3 lg:grid-cols-6 gap-3">
      <?php foreach($prayer_time as $shalat) : ?>
        <div class="bg-white dark:bg-dark-secondary py-5 px-4 shadow rounded-lg flex flex-col bg-flower bg-right-top bg-5 bg-no-repeat space-y-1">
          ...
          <span class="text-2xl font-bold shalat-time lg:ml-1" data-name="<?= $shalat ?>"><span class="animate-spin inline-block"><i class="ti ti-loader-2 text-xl"></i></span></span>
        </div>
      <?php endforeach ?>
    </div>
  </section>
