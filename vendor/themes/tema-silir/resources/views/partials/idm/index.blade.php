...
@section('content')
  <div class="main-content space-y-4">
    <h1 class="text-heading text-lg lg:text-2xl border-b border-dotted border-primary dark:border-white pb-2 title"><i class=" ti ti-chart-pie inline-block mr-1"></i> Status IDM {{ $tahun }}</h1>
    <div class="bg-slate-200 dark:bg-dark-primary py-3 px-5 border-l-4 border-secondary my-5 space-y-3">
      <p class="text-sm text-justify">Indeks Desa Membangun (IDM) merupakan indeks komposit yang dibentuk dari tiga indeks, yaitu Indeks Ketahanan Sosial, Indeks Ketahanan Ekonomi, dan Indeks Ketahanan Ekologi/Lingkungan. Nilai IDM yang semakin tinggi menunjukkan kondisi desa yang semakin baik dari segi sosial, ekonomi, dan ekologi. IDM dapat menentukan status desa berdasarkan nilai dari indeks-indeks tersebut.</p>
    </div>
    @if($idm->error_msg)
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-5 max-w-full">
      <div class="rounded overflow-hidden bg-blue-500 relative text-white py-5 px-3 lg:px-4">
        ...
        <div class="icon absolute right-0 mr-5 text-5xl text-slate-300 text-opacity-30 top-1/2 transform -translate-y-1/2">
        ...
        <table class="overflow-auto table-striped table text-sm capitalize">
          <tbody>
            <tr>
              ...
              <td id="nama-provinsi">@include('theme::commons.loading')</td>
            </tr>
            <tr>
              <th class="horizontal">KABUPATEN</th>
              <td id="nama-kabupaten">@include('theme::commons.loading')</td>
            </tr>
            <tr>
              <th class="horizontal">{{ strtoupper(setting('sebutan_kecamatan')) }}</th>
              <td id="nama-kecamatan">@include('theme::commons.loading')</td>
            </tr>
            <tr>
              <th class="horizontal">{{ strtoupper(setting('sebutan_desa')) }}</th>
              <td id="nama-desa">@include('theme::commons.loading')</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="table-responsive overflow-auto w-full text-xs">
...
@push('scripts')
  <script>
    $(document).ready(function () {
      const tahun = '{{ $tahun }}';
      const route = "{{ route('api.idm', $tahun) }}";
      ...
      $.get(route, function (data) {
        ...
        $('#status-idm').show();
        $('#status-error').hide();
        $('#tabel-daftar tbody').empty();

        const summaries = data['data'][0]['attributes']['SUMMARIES'];
        const row = data['data'][0]['attributes']['ROW'];
        const identitas = data['data'][0]['attributes']['IDENTITAS'][0];
