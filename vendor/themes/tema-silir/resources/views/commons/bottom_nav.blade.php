...
<section class="fixed inset-x-0 bottom-0 left-0 max-w-screen right-0 z-40 bg-white dark:bg-dark-secondary shadow-xl lg:hidden lg:invisible bottom-nav">
  <div class="flex justify-between">
    @foreach($navs as $nav) 
      <a href="{{ $nav['url'] }}" class="flex flex-col items-center w-full pt-2 pb-1 active:text-secondary @php $nav['icon'] == 'category-2' and print('bg-primary text-white') @endphp" data-mdb-ripple="true" data-mdb-ripple-color="light" @php $nav['icon'] == 'category-2' and print('@click="drawer = !drawer"') @endphp
        @if($nav['icon'] == 'login') 
