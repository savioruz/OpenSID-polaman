            ...
            <div class="collapse box-body no-padding" id="collapseStatGraph">
              <div class="w-full divide-y dark:divide-dark-primary text-sm">
                <?php foreach ($list_ref as $key => $value) : ?>
                  <li <?= jecho($lap, $key, 'class="active"'); ?>><a class="py-1 block" href="<?= site_url("statistik_web/chart_gis_rw/{$key}/" . underscore($rw['dusun']) . '/' . underscore($rw['rw'])) ?>" data-bs-remote="false" data-bs-toggle="modal" data-bs-target="#modalContent" data-title="Statistik Penduduk RW <?= set_ucwords($rw['rw']) . ' ' . set_ucwords($wilayah) . set_ucwords($rw['dusun']) ?>"><?= $value ?></a>
                  </li>
                <?php endforeach; ?>
              </div>
            </div>
            ...
            <div class="collapse box-body no-padding" id="collapseStatBantuan">
              <div class="w-full divide-y dark:divide-dark-primary text-sm">
                <?php foreach ($list_bantuan as $key => $value) : ?>
                  <li <?= jecho($lap, $key, 'class="active"'); ?>><a class="py-1 block" href="<?= site_url("statistik_web/chart_gis_rw/{$key}/" . underscore($rw['dusun']) . '/' . underscore($rw['rw'])) ?>" data-bs-remote="false" data-bs-toggle="modal" data-bs-target="#modalContent" data-title="Statistik Bantuan RW <?= set_ucwords($rw['rw']) . ' ' . set_ucwords($wilayah) . set_ucwords($rw['dusun']) ?>"><?= $value ?></a>
                  </li>
                <?php endforeach; ?>
              </div>
            </div>
