@php 
  $shortcuts = [
    [
      'icon' => 'map.png',
      'name' => 'Peta ' . ucwords(setting('sebutan_desa')),
      'link' => site_url('peta'),
      'is_external' => false
    ],
    [
      'icon' => 'compliant.png',
      'name' => 'Produk Hukum',
      'link' => site_url('peraturan-desa'),
      'is_external' => false
    ],
    [
      'icon' => 'document.png',
      'name' => 'Informasi Publik',
      'link' => site_url('informasi-publik'),
      'is_external' => false
    ],
    [
      'icon' => 'trolley.png',
    ...
    [
      'icon' => 'pie-chart.png',
      'name' => 'Status IDM',
      'link' => site_url('status-idm/' . (date('Y') - 1)),
      'is_external' => false
    ]];
@endphp
...
<section class="grid gap-4 lg:gap-5 grid-cols-3 md:grid-cols-3 lg:grid-cols-9 my-5" id="shortcut-link">
  @foreach($shortcuts as $index => $shortcut) 
    <a href="{{ $shortcut['link'] }}" target="@php $shortcut['is_external'] and print('_blank') }}" rel="noopener" class="card shadow-lg px-3 py-2 lg:p-3 space-y-2 group shortcut-card dark:border dark:border-gray-500" data-aos="zoom-in" data-aos-delay="{{ $index * 50 }}">
      <img loading="lazy" src="{{ theme_asset("images/{$shortcut['icon']}") }}"" alt="{{ $shortcut['name'] }}" class="w-auto mx-auto py-2 h-12 group-hover:scale-125 transition duration-200">
