...
@section('content')
  ...
  <div class="flex space-x-2">
      <a href="http://www.facebook.com/sharer.php?u={{ site_url('artikel/'.buat_slug($post)) }}" target="_blank" rel="noopener" class="inline-flex items-center justify-center w-10 h-10 bg-facebook text-white rounded-full hover:ring-2 hover:ring-tertiary hover:ring-offset-2 transition duration-100">
      ...
      <a href="https://telegram.me/share/url?url={{ site_url('artikel/'.buat_slug($post)) }}" target="_blank" rel="noopener" class="inline-flex items-center justify-center w-10 h-10 bg-telegram text-white rounded-full hover:ring-2 hover:ring-tertiary hover:ring-offset-2 transition duration-100">
        <i class="ti ti-brand-telegram text-xl"></i>
      </a>
      <a href="https://api.whatsapp.com/send?text={{ site_url('artikel/'.buat_slug($post)) }}" target="_blank" rel="noopener" class="inline-flex items-center justify-center w-10 h-10 bg-whatsapp text-white rounded-full hover:ring-2 hover:ring-tertiary hover:ring-offset-2 transition duration-100">
...
@push('scripts')
  <script>
    function printArticle() {
      ...
      pri.document.write(document.querySelector('.print').innerHTML);
      pri.document.write('<p>Baca artikel online: <a href="{{ site_url('artikel/'.buat_slug($post))}}" class="text-link">{{ site_url('artikel/'.buat_slug($post)) }}</a></p>')
      pri.document.close();
      setTimeout(() => {
        pri.focus();
        pri.print();
