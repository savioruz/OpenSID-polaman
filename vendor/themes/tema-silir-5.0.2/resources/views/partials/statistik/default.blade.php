...
<div class="flex flex-col lg:flex-row justify-between items-center space-x-1">
  <h2 class="text-heading text-base lg:text-xl w-auto">Grafik {{ $heading }}</h2>
...
<h2 class="text-heading text-base lg:text-xl">Tabel {{ $heading }}</h2>
<div class="table-responsive" id="statistic-table">
...
@if(setting('daftar_penerima_bantuan') && $bantuan)
  ...
  <h2 class="text-heading text-base lg:text-xl">Daftar {{ $heading }}</h2>
...
@push('scripts')
  <script>
    const statistikUrl = `{{ ci_route('internal_api.statistik', $key) }}?filter[tahun]={{ $selected_tahun ?? '' }}`;
  </script>
  <script src="{{ theme_asset('lib/js/statistik.min.js') }}"></script>
@endpush