@if($paging->num_rows > $paging->per_page) 
  ...
  <nav aria-label="nomor halaman">
    <ul class="pagination mt-1 mb-5">
      ...
      @endif
      @for($page = $p-2; $page <= $p+3; $page++) 
        @if($page > 0 && $page <= $paging->end_link) 
          <li>
            <a href="{{ site_url($paging_page.'/'.$page.$paging->suffix) }}" class="pagination-link @php ($p == $page || $paging->page == $page) and print('is-active') @endphp">{{ $page }}</a>
          </li>
        @endif
      @endfor
      @if($paging->next) 
