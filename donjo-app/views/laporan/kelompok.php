...
<div class="content-wrapper">
	...
	<section class="content">
		<div class="row">
			...
			// Initialize total_cacat array with zeros
			...
			foreach ($main as $data): $id_cluster = $data['id_cluster']; ?>
											<tr>
												...
												<?php
			        $bayi = (int)$bayi + (int)($data['bayi'] ?? 0);
			    $balita   = (int)$balita + (int)($data['balita'] ?? 0);
			    $sd       = (int)$sd + (int)($data['sd'] ?? 0);
			    $smp      = (int)$smp + (int)($data['smp'] ?? 0);
			    $sma      = (int)$sma + (int)($data['sma'] ?? 0);
			    $lansia   = (int)$lansia + (int)($data['lansia'] ?? 0);
			    $cacat    = (int)$cacat + (int)($data['cacat'] ?? 0);
			    $sakit_L  = (int)$sakit_L + (int)($data['sakit_L'] ?? 0);
			    $sakit_P  = (int)$sakit_P + (int)($data['sakit_P'] ?? 0);
			    $hamil    = (int)$hamil + (int)($data['hamil'] ?? 0);

			    foreach ($jenis_cacat as $key => $val):
			        $total_cacat[$key] = (int)$total_cacat[$key] + (int)($data[$val] ?? 0);
			    endforeach;
			    ?>
											</tr>
										<?php endforeach; ?>
