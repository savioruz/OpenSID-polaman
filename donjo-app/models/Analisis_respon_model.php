...
class Analisis_respon_model extends MY_Model
{
        ...
        switch ($this->subjek) {
            ...
            case 2:
                $this->config_id('u')
                    ...
                    ->join('tweb_penduduk p', 'u.nik_kepala = p.id AND p.status_dasar = 1', 'left')
            ...
            case 4:
                $this->config_id('u')
                    ...
                    ->from('kelompok u')
                    ->join('tweb_penduduk p', 'u.id_ketua = p.id AND p.status_dasar = 1', 'left')
                    ->join('tweb_wil_clusterdesa c', 'p.id_cluster = c.id', 'left');
                break;
        ...
        switch ($this->subjek) {
            ...
            case 3:
                ...
                $this->db
                    ...
                    ->or_like('p.nama', $cari)
                    ->group_end()
                    ->or_where("(SELECT COUNT(id) FROM tweb_penduduk WHERE status_dasar = 1 AND nik LIKE '{$kw}' AND id_rtm = u.id AND config_id = '{$this->config_id}') > 1")
                    ->or_where("(SELECT COUNT(id) FROM tweb_penduduk WHERE status_dasar = 1 AND nama LIKE '{$kw}' AND id_rtm = u.id AND config_id = '{$this->config_id}') > 1")
                    ->group_end();
                break;
    ...
    public function get_subjek($id = 0)
    {
        ...
        if ($this->subjek == 5) {
            $tambahan = [
                'jumlah_total_penduduk'            => $this->config_id()->where('status_dasar', 1)->count_all_results('tweb_penduduk'),
                'jumlah_penduduk_laki_laki'        => $this->config_id()->where('sex', 1)->where('status_dasar', 1)->count_all_results('tweb_penduduk'),
                'jumlah_penduduk_perempuan'        => $this->config_id()->where('sex', 2)->where('status_dasar', 1)->count_all_results('tweb_penduduk'),
                'jumlah_penduduk_pedatang'         => $this->config_id()->where('status', 2)->where('status_dasar', 1)->count_all_results('tweb_penduduk'),
                'jumlah_penduduk_yang_pergi'       => $this->config_id()->where('kode_peristiwa', 3)->count_all_results('log_penduduk'),
                'jumlah_total_kepala_keluarga'     => $this->config_id('u')->join('tweb_penduduk t', 'u.nik_kepala = t.id AND t.status_dasar = 1', 'left')->where('u.tgl_cetak_kk', '!=', '')->count_all_results('tweb_keluarga u'),
                'jumlah_kepala_keluarga_laki_laki' => $this->config_id('u')->join('tweb_penduduk t', 'u.nik_kepala = t.id AND t.status_dasar = 1', 'left')->where('sex', 1)->where('u.tgl_cetak_kk', '!=', '')->count_all_results('tweb_keluarga u'),
                'jumlah_kepala_keluarga_perempuan' => $this->config_id('u')->join('tweb_penduduk t', 'u.nik_kepala = t.id AND t.status_dasar = 1', 'left')->where('sex', 2)->where('u.tgl_cetak_kk', '!=', '')->count_all_results('tweb_keluarga u'),
                'jumlah_peserta_bpjs'              => $this->config_id()->where('bpjs_ketenagakerjaan != ', null)->where('status_dasar', 1)->count_all_results('tweb_penduduk'),
            ];
