...
class Laporan_penduduk_model extends MY_Model
{
    ...
    {
        $sql = '(SELECT COUNT(b.id) FROM tweb_penduduk b
		LEFT JOIN tweb_wil_clusterdesa a ON b.id_cluster = a.id
		WHERE b.status_dasar = 1 ';
        $sql .= $fk ? "AND {$fk} = u.id " : '';
        $sql .= $where ?: '';
        $sql .= $this->dusun_sql();
        $sql .= $this->rw_sql();
    ...
    protected function get_laki_sql($fk = false, $delimiter = false, $where = 0)
    {
        $sql = '(SELECT COUNT(b.id) FROM tweb_penduduk b
		LEFT JOIN tweb_wil_clusterdesa a ON b.id_cluster = a.id
		WHERE b.status_dasar = 1 AND sex = 1 ';
        $sql .= $fk ? "AND {$fk} = u.id " : '';
        $sql .= $where ?: '';
        $sql .= $this->dusun_sql();
        $sql .= $this->rw_sql();
    ...
    protected function get_perempuan_sql($fk = false, $delimiter = false, $where = 0)
    {
        $sql = '(SELECT COUNT(b.id) FROM tweb_penduduk b
		LEFT JOIN tweb_wil_clusterdesa a ON b.id_cluster = a.id
		WHERE b.status_dasar = 1 AND sex = 2 ';
        $sql .= $fk ? "AND {$fk} = u.id " : '';
        $sql .= $where ?: '';
        $sql .= $this->dusun_sql();
        $sql .= $this->rw_sql();
    ...
    private function select_jml_penduduk_per_kategori($id_referensi, $tabel_referensi)
    {
        ...
        $this->db
            ...
            ->join('tweb_penduduk p', "u.id = p.{$id_referensi} AND p.config_id = {$this->config_id} AND p.status_dasar = 1", 'left')
        ...
        switch ("{$lap}") {
            ...
            case 'kelas_sosial':
                ...
                $this->db
                    ...
                    ->join('tweb_keluarga k', "k.kelas_sosial = u.id AND k.config_id = {$this->config_id} AND k.tgl_cetak_kk != ''", 'left')
                    ->join('tweb_penduduk p', 'p.id=k.nik_kepala', 'left')
                    ->group_by('u.id');
                break;
