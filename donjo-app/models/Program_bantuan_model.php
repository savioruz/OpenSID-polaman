...
class Program_bantuan_model extends MY_Model
{
    ...
    private function get_pilihan_kk($filter)
    {
        ...
        $query  = $this->config_id('p')
            ...
            ->join('tweb_keluarga k', 'k.id = p.id_kk AND k.tgl_cetak_kk != ""', 'OUTER JOIN')
    ...
    //Ambil data yg dibutuhkan saja, ambil dr tabel tweb_penduduk
    ...
    public function get_kk($id_kk)
    {
        return $this->config_id('k')
            ...
            ->join('tweb_penduduk p', 'p.id = k.nik_kepala AND p.status_dasar = 1', 'left')
        ...
        switch ($sasaran) {
            ...
            case 2:
                ...
                $data = $this->config_id('p')
                    ...
                    ->from('tweb_penduduk p')
                    ->where('p.status_dasar', 1)
                    ->join('tweb_keluarga k', 'k.id = p.id_kk AND k.tgl_cetak_kk != ""', 'left')
