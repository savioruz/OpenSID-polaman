...
class Arsip_fisik_model extends MY_Model
{
    ...
    public function ambil_dokumen_per_page($limit = true, $per_page = 50, $p = 1, $o = 4)
    {
        ...
        $query_dokumen_desa = $this->config_id_exist('dokumen')
            ->select('`id` as id')
            ...
            ->from('dokumen')
            ->where('`id_pend` = 0 AND `satuan` IS NOT NULL AND `deleted` <> 1')
            ->get_compiled_select();
        ...
        $query_kependudukan = $this->config_id_exist('dokumen', 'd')
            ->select('d.`id` as id')
            ...
            ->where('d.`id_pend` !=0 AND d.`satuan` IS NOT NULL AND d.`deleted` <> 1')
            ->get_compiled_select();
