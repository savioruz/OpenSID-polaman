...
class Wilayah_model extends MY_Model
{
    ...
    private function list_data_sql()
    {
        $sql = " FROM tweb_wil_clusterdesa u
			LEFT JOIN tweb_penduduk a ON u.id_kepala = a.id AND a.status_dasar = 1
    ...
    */
    public function list_data($o = 0, $offset = 0, $limit = 500)
    {
		...
		(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun)) AS jumlah_warga,
		(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun) AND p.sex = 1) AS jumlah_warga_l,
		(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun) AND p.sex = 2) AS jumlah_warga_p,
		(SELECT COUNT(p.id) FROM tweb_keluarga k inner join tweb_penduduk p ON k.nik_kepala = p.id  WHERE k.tgl_cetak_kk != '' AND p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun) AND p.kk_level = 1) AS jumlah_kk";
    ...
    public function list_semua_wilayah()
    {
        ...
        return $this->config_id('w')
            ...
            ->join('tweb_penduduk p', 'w.id_kepala = p.id AND p.status_dasar = 1', 'left')
    ...
    private function select_jumlah_warga()
    {
        $this->db
            ->select('(CASE
				WHEN ' . $this->case_dusun . ' THEN (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = w.dusun))
				WHEN ' . $this->case_rw . ' THEN (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = w.dusun and rw = w.rw))
				WHEN ' . $this->case_rt . ' THEN (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster = w.id)
				END) AS jumlah_warga');

        $this->db
            ->select('(CASE
				WHEN ' . $this->case_dusun . ' THEN (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = w.dusun) AND p.sex = 1)
				WHEN ' . $this->case_rw . ' THEN (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = w.dusun and rw = w.rw) AND p.sex = 1)
				WHEN ' . $this->case_rt . ' THEN (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster = w.id and p.sex = 1)
				END) AS jumlah_warga_l');

        $this->db
            ->select('(CASE
				WHEN ' . $this->case_dusun . ' THEN (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = w.dusun) AND p.sex = 2)
				WHEN ' . $this->case_rw . ' THEN (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = w.dusun and rw = w.rw) AND p.sex = 2)
				WHEN ' . $this->case_rt . ' THEN (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster = w.id and p.sex = 2)
    ...
    private function select_jumlah_kk()
    {
        $this->db
            ->select('(CASE
				WHEN ' . $this->case_dusun . ' THEN (SELECT COUNT(p.id) FROM tweb_keluarga k inner join tweb_penduduk p ON k.nik_kepala = p.id WHERE k.tgl_cetak_kk != "" AND p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = w.dusun) AND p.kk_level = 1)
				WHEN ' . $this->case_rw . ' THEN (SELECT COUNT(p.id) FROM tweb_keluarga k inner join tweb_penduduk p ON k.nik_kepala = p.id WHERE k.tgl_cetak_kk != "" AND p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE dusun = w.dusun and rw = w.rw) AND p.kk_level = 1)
				WHEN ' . $this->case_rt . ' THEN (SELECT COUNT(p.id) FROM tweb_keluarga k inner join tweb_penduduk p ON k.nik_kepala = p.id WHERE k.tgl_cetak_kk != "" AND p.status_dasar = 1 AND p.id_cluster = w.id AND p.kk_level = 1)
				END) AS jumlah_kk ');
    }
    ...
    public function list_data_rw($id = '', $offset = 0, $limit = 0)
    {
        ...
        (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun AND c.rw = u.rw)) AS jumlah_warga,
        (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun AND c.rw = u.rw) AND p.sex = 1) AS jumlah_warga_l,
        (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun AND c.rw = u.rw) AND p.sex = 2) AS jumlah_warga_p,
        (SELECT COUNT(p.id) FROM tweb_keluarga k inner join tweb_penduduk p ON k.nik_kepala=p.id  WHERE k.tgl_cetak_kk != '' AND p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun AND c.rw = u.rw) AND p.kk_level = 1) AS jumlah_kk";

        $this->config_id('u')
            ->select($select_sql)
            ->from('tweb_wil_clusterdesa u')
            ->join('tweb_penduduk a', 'u.id_kepala = a.id AND a.status_dasar = 1', 'LEFT')
            ->where('u.rt', '0')
            ->where('u.rw <>', '0')
    ...
    public function list_data_rt($dusun = '', $rw = '', $offset = 0, $limit = 0)
    {
        $this->list_data_rt_query($dusun, $rw);
        $select_sql = "u.*, a.nama AS nama_ketua, a.nik AS nik_ketua,
        (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun AND c.rw = u.rw AND rt = u.rt)) AS jumlah_warga,
        (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun AND c.rw = u.rw AND rt = u.rt) AND p.sex = 1) AS jumlah_warga_l,
        (SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun AND c.rw = u.rw AND rt = u.rt) AND p.sex = 2) AS jumlah_warga_p,
        (SELECT COUNT(p.id) FROM tweb_keluarga k inner join tweb_penduduk p ON k.nik_kepala=p.id  WHERE k.tgl_cetak_kk != '' AND p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun AND c.rw = u.rw AND rt = u.rt) AND p.kk_level = 1) AS jumlah_kk";
    ...
    public function total()
    {
		...
		(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id)) AS total_warga,
		(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id) AND p.sex = 1) AS total_warga_l,
		(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id) AND p.sex = 2) AS total_warga_p,
		(SELECT COUNT(p.id) FROM tweb_keluarga k inner join tweb_penduduk p ON k.nik_kepala=p.id WHERE k.tgl_cetak_kk != '' AND p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id) AND p.kk_level = 1) AS total_kk
		FROM tweb_wil_clusterdesa u
		LEFT JOIN tweb_penduduk a ON u.id_kepala = a.id AND a.status_dasar = 1 WHERE u.rt = '0' AND u.rw = '0' AND u.config_id = '" . identitas('id') . "' limit 1";
        $query = $this->db->query($sql);
    ...
    public function total_rw($dusun = '')
    {
        $sql = "SELECT sum(jumlah_rt) AS jmlrt, sum(jumlah_warga) AS jmlwarga, sum(jumlah_warga_l) AS jmlwargal, sum(jumlah_warga_p) AS jmlwargap, sum(jumlah_kk) AS jmlkk
			...
			(SELECT u.*, a.nama AS nama_ketua, a.nik AS nik_ketua,
				...
				(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun AND c.rw = u.rw) AND p.sex = 1) AS jumlah_warga_l,
				(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND c.dusun = u.dusun AND c.rw = u.rw) AND p.sex = 2) AS jumlah_warga_p,
				(SELECT COUNT(p.id) FROM  tweb_keluarga k inner join tweb_penduduk p ON k.nik_kepala=p.id WHERE k.tgl_cetak_kk != '' AND p.status_dasar = 1 AND p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa c WHERE c.config_id = u.config_id AND dusun = u.dusun AND c.rw = u.rw) AND p.kk_level = 1) AS jumlah_kk
				FROM tweb_wil_clusterdesa u
				LEFT JOIN tweb_penduduk a ON u.id_kepala = a.id AND a.status_dasar = 1
