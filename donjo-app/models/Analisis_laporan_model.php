    ...
    public function autocomplete()
    {
        $sql = 'SELECT no_kk FROM tweb_keluarga WHERE config_id = ' . identitas('id') .
            ' UNION SELECT t.nama
                ...
                LEFT JOIN tweb_penduduk t ON u.nik_kepala = t.id AND t.status_dasar = 1
        ...
        switch ($this->subjek) {
            ...
            case 3:
                ...
                $this->db
                    ...
                    ->or_like('p.nama', $cari)
                    ->group_end()
                    ->or_where("(SELECT COUNT(id) FROM tweb_penduduk WHERE status_dasar = 1 AND nik LIKE '{$kw}' AND id_rtm = u.id) > 1")
                    ->or_where("(SELECT COUNT(id) FROM tweb_penduduk WHERE status_dasar = 1 AND nama LIKE '{$kw}' AND id_rtm = u.id) > 1")
                    ->group_end();
                break;
    ...
    public function list_data_query()
    {
        ...
        switch ($this->subjek) {
            ...
            case 3:
                $this->config_id('u')
                    ->from('tweb_rtm u')
                    ->join('tweb_penduduk p', 'u.nik_kepala = p.id AND p.status_dasar = 1')
