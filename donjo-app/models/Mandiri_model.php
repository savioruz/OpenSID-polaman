...
class Mandiri_model extends MY_Model
{
    ...
    public function autocomplete()
    {
        $data = $this->config_id('pm')
            ...
            ->join('tweb_penduduk p', 'p.id = pm.id_pend AND p.status_dasar = 1', 'left')
            ->limit(15)
            ->get()
            ->result_array();
    ...
    private function list_data_sql()
    {
        $this->config_id('pm')
            ->from('tweb_penduduk_mandiri pm')
            ->join('tweb_penduduk p', 'pm.id_pend = p.id AND p.status_dasar = 1');
    ...
    public function update($id_pend = null)
    {
        ...
        $post = $this->input->post();
        $pin  = bilangan($post['pin'] ?? $this->generate_pin());
        $nama = $this->config_id()->select('nama')->where('id', $id_pend)->where('status_dasar', 1)->get('tweb_penduduk')->row()->nama;
    ...
    private function list_data_ajax_sql($cari = '')
    {
        $this->config_id('u')
            ...
            ->join('tweb_penduduk n', 'u.id_pend = n.id AND n.status_dasar = 1', 'left')
