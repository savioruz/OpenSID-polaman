...
class Laporan_bulanan_model extends MY_Model
{
    ...
    public function list_data()
    {
        $sql = "select c.id as id_cluster,c.rt,c.rw,c.dusun as dusunnya,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and sex='1' and id_cluster=c.id) as L,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and sex='2' and id_cluster=c.id) as P,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and (DATE_FORMAT( FROM_DAYS( TO_DAYS(NOW()) - TO_DAYS( tanggallahir ) ) , '%Y' ) +0)<1 and id_cluster=c.id ) as bayi,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and (DATE_FORMAT( FROM_DAYS( TO_DAYS(NOW()) - TO_DAYS( tanggallahir ) ) , '%Y' ) +0)>=1 and (DATE_FORMAT( FROM_DAYS( TO_DAYS(NOW()) - TO_DAYS( tanggallahir ) ) , '%Y' ) +0)<=5  and id_cluster=c.id ) as balita,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and (DATE_FORMAT( FROM_DAYS( TO_DAYS(NOW()) - TO_DAYS( tanggallahir ) ) , '%Y' ) +0)>=6 and (DATE_FORMAT( FROM_DAYS( TO_DAYS(NOW()) - TO_DAYS( tanggallahir ) ) , '%Y' ) +0)<=12  and id_cluster=c.id ) as sd,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and (DATE_FORMAT( FROM_DAYS( TO_DAYS(NOW()) - TO_DAYS( tanggallahir ) ) , '%Y' ) +0)>=13 and (DATE_FORMAT( FROM_DAYS( TO_DAYS(NOW()) - TO_DAYS( tanggallahir ) ) , '%Y' ) +0)<=15  and id_cluster=c.id ) as smp,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and (DATE_FORMAT( FROM_DAYS( TO_DAYS(NOW()) - TO_DAYS( tanggallahir ) ) , '%Y' ) +0)>=16 and (DATE_FORMAT( FROM_DAYS( TO_DAYS(NOW()) - TO_DAYS( tanggallahir ) ) , '%Y' ) +0)<=18  and id_cluster=c.id ) as sma,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and (DATE_FORMAT( FROM_DAYS( TO_DAYS(NOW()) - TO_DAYS( tanggallahir ) ) , '%Y' ) +0)>60 and id_cluster=c.id ) as lansia,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and id_cluster=c.id and cacat_id in (1,2,3,4,5,6)) as cacat,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and id_cluster=c.id and cacat_id='1') as cacat_fisik,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and id_cluster=c.id and cacat_id='2') as cacat_netra,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and id_cluster=c.id and cacat_id='3') as cacat_rungu,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and id_cluster=c.id and cacat_id='4') as cacat_mental,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and id_cluster=c.id and cacat_id='5') as cacat_fisik_mental,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and id_cluster=c.id and cacat_id='6') as cacat_lainnya,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and id_cluster=c.id and (cacat_id IS NULL OR cacat_id='7')) as tidak_cacat,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and sakit_menahun_id is not null and sakit_menahun_id <>'0' and sakit_menahun_id <>'14' and id_cluster=c.id and sex='1') as sakit_L,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and sakit_menahun_id is not null and sakit_menahun_id <>'0' and sakit_menahun_id <>'14' and id_cluster=c.id and sex='2') as sakit_P,
            (select count(id) from tweb_penduduk where config_id=c.config_id and status_dasar=1 and hamil='1' and id_cluster=c.id) as hamil
