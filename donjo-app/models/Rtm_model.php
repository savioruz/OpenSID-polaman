...
class Rtm_model extends MY_Model
{
    ...
    public function get_kepala_rtm($id, $is_no_kk = false)
    {
        ...
        $data = $this->config_id('r')
            ->select("u.id, u.nik, u.nama, u.status_dasar, r.no_kk, r.bdt, x.nama AS sex, u.tempatlahir, u.tanggallahir, (SELECT DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW()) - TO_DAYS(`tanggallahir`)), '%Y') + 0 FROM tweb_penduduk WHERE id = u.id AND status_dasar = 1) AS umur, d.nama as pendidikan, f.nama as warganegara, a.nama as agama, wil.rt, wil.rw, wil.dusun")
            ->from('tweb_rtm r')
            ->join('tweb_penduduk u', 'r.no_kk = u.id_rtm AND u.rtm_level = 1 AND u.status_dasar = 1', 'left')
            ->join('tweb_penduduk_sex x', 'u.sex = x.id', 'left')
            ->join('tweb_penduduk_pendidikan_kk d', 'u.pendidikan_kk_id = d.id', 'left')
            ->join('tweb_penduduk_warganegara f', 'u.warganegara_id = f.id', 'left')
            ->join('tweb_penduduk_agama a', 'u.agama_id = a.id', 'left')
    ...
    public function autocomplete()
    {
        $this->config_id('u')
            ->select('t.nama')
            ->from('tweb_rtm u')
            ->join('tweb_penduduk t', 'u.nik_kepala = t.id AND t.status_dasar = 1', 'LEFT');
    ...
    public function list_data($page = 1)
    {
        ...
        $this->config_id('u')
            ...
            ->select('(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.status_dasar = 1 AND p.id_rtm = u.no_kk ) AS jumlah_anggota')
            ->from("({$query_dasar}) as u")
            ->join('tweb_penduduk t', 'u.no_kk = t.id_rtm AND t.rtm_level = 1 AND t.status_dasar = 1', 'left')
