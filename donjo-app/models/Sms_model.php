...
class Sms_model extends MY_Model
{
    ...
    {
        return " FROM inbox i
            LEFT JOIN tweb_penduduk p on i.SenderNumber = p.telepon AND p.status_dasar = 1
    ...
    private function list_data_terkirim_sql()
    {
        return " FROM sentitems s
            LEFT JOIN tweb_penduduk p on s.DestinationNumber = p.telepon AND s.config_id = p.config_id AND p.status_dasar = 1
    ...
    private function list_data_tertunda_sql()
    {
        return " FROM outbox u
            LEFT JOIN tweb_penduduk p on u.DestinationNumber = p.telepon AND u.config_id = p.config_id AND p.status_dasar = 1
